...
requireRole(['admin', 'manager', 'accountant', 'feed_worker']);
...
// ── Inline Migrations ─────────────────────────────────────────────────────────
$fi_cols = array_column($db->query("SHOW COLUMNS FROM feed_inventory")->fetchAll(), 'Field');
if (!in_array('category', $fi_cols))
    $db->exec("ALTER TABLE feed_inventory ADD COLUMN category VARCHAR(100) DEFAULT NULL AFTER item_name");
if (!in_array('selling_price', $fi_cols))
    $db->exec("ALTER TABLE feed_inventory ADD COLUMN selling_price DECIMAL(10,2) DEFAULT NULL AFTER purchase_price");
if (!in_array('expiry_date', $fi_cols))
    $db->exec("ALTER TABLE feed_inventory ADD COLUMN expiry_date DATE DEFAULT NULL");
if (!in_array('created_by', $fi_cols))
    $db->exec("ALTER TABLE feed_inventory ADD COLUMN created_by INT UNSIGNED DEFAULT NULL");
if (!in_array('updated_by', $fi_cols))
    $db->exec("ALTER TABLE feed_inventory ADD COLUMN updated_by INT UNSIGNED DEFAULT NULL");

$fs_cols = array_column($db->query("SHOW COLUMNS FROM feed_sales")->fetchAll(), 'Field');
if (!in_array('buyer_phone', $fs_cols))
    $db->exec("ALTER TABLE feed_sales ADD COLUMN buyer_phone VARCHAR(30) DEFAULT NULL AFTER buyer_name");
if (!in_array('buyer_address', $fs_cols))
    $db->exec("ALTER TABLE feed_sales ADD COLUMN buyer_address VARCHAR(255) DEFAULT NULL AFTER buyer_phone");
if (!in_array('payment_method', $fs_cols))
    $db->exec("ALTER TABLE feed_sales ADD COLUMN payment_method ENUM('cash','bank','mobile_banking','credit','other') NOT NULL DEFAULT 'cash'");
if (!in_array('payment_status', $fs_cols))
    $db->exec("ALTER TABLE feed_sales ADD COLUMN payment_status ENUM('paid','pending','partial') NOT NULL DEFAULT 'paid'");
if (!in_array('amount_paid', $fs_cols))
    $db->exec("ALTER TABLE feed_sales ADD COLUMN amount_paid DECIMAL(12,2) NOT NULL DEFAULT 0.00");
if (!in_array('cost_price', $fs_cols))
    $db->exec("ALTER TABLE feed_sales ADD COLUMN cost_price DECIMAL(12,2) NOT NULL DEFAULT 0.00");
if (!in_array('profit', $fs_cols))
    $db->exec("ALTER TABLE feed_sales ADD COLUMN profit DECIMAL(12,2) NOT NULL DEFAULT 0.00");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCsrfToken($_POST[CSRF_TOKEN_NAME] ?? '')) {
        ...
        redirect('/modules/inventory/feed.php');
    }
    $action = $_POST['action'] ?? '';
    ...
    if (in_array($action, ['add_item', 'edit_item'], true)) {
        ...
        $buy_price  = (float)($_POST['purchase_price'] ?? 0);
        ...
        if (empty($errors)) {
            if ($action === 'add_item') {
                $db->prepare("INSERT INTO feed_inventory (farm_id, item_name, category, quantity, unit, purchase_price, selling_price, supplier, purchase_date, reorder_threshold, expiry_date, created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)")
                   ->execute([$fid, $item_name, $category ?: null, $qty, $unit, $buy_price ?: null, $sell_price, $supplier ?: null, $pur_date, $reorder, $expiry, $uid]);
                ...
                if ($qty > 0) {
                    $db->prepare("INSERT INTO inventory_transactions (farm_id,item_type,item_id,item_name,transaction_type,quantity,unit,unit_cost,total_value,notes,recorded_by) VALUES (?,?,?,?,?,?,?,?,?,?,?)")
                       ->execute([$fid, 'feed', $new_id, $item_name, 'purchase', $qty, $unit, $buy_price ?: null, $buy_price > 0 ? round($qty * $buy_price, 2) : null, 'Initial stock', $uid]);
                    if ($buy_price > 0) {
                        $db->prepare("INSERT INTO finance_transactions (farm_id,type,category,amount,related_module,reference_id,transaction_date,recorded_by,notes) VALUES (?,?,?,?,?,?,?,?,?)")
                           ->execute([$fid, 'expense', 'Feed Purchase', round($qty * $buy_price, 2), 'feed', $new_id, $pur_date ?? $today, $uid, "Initial stock: {$qty} {$unit} of {$item_name}"]);
                    }
                }
                auditLog($uid, 'ADD_FEED_ITEM', 'feed_inventory', $new_id, [], ['name' => $item_name, 'qty' => $qty]);
                flashMessage('success', "'{$item_name}' added to feed stock.");
            } else {
                $old = $db->prepare("SELECT * FROM feed_inventory WHERE id = ? AND farm_id = ?");
                $old->execute([$edit_id, $fid]);
                $old_row = $old->fetch();
                if ($old_row) {
                    $db->prepare("UPDATE feed_inventory SET item_name=?,category=?,supplier=?,unit=?,purchase_price=?,selling_price=?,reorder_threshold=?,expiry_date=?,purchase_date=?,updated_by=? WHERE id=? AND farm_id=?")
                       ->execute([$item_name, $category ?: null, $supplier ?: null, $unit, $buy_price ?: null, $sell_price, $reorder, $expiry, $pur_date, $uid, $edit_id, $fid]);
                    auditLog($uid, 'EDIT_FEED_ITEM', 'feed_inventory', $edit_id, $old_row, ['name' => $item_name]);
                    flashMessage('success', "'{$item_name}' updated.");
                } else {
                    $errors[] = 'Item not found.';
                }
            }
            if (empty($errors)) redirect('/modules/inventory/feed.php?tab=stock');
        }
    ...
    if ($action === 'purchase') {
        ...
        if ($item_id <= 0) $errors[] = 'Please select a feed item.';
        if ($qty <= 0)      $errors[] = 'Quantity must be greater than zero.';

        $item = null;
        if (empty($errors)) {
            $s = $db->prepare("SELECT * FROM feed_inventory WHERE id = ? AND farm_id = ?");
        ...
        if (empty($errors)) {
            try {
                $db->beginTransaction();
                $db->prepare("UPDATE feed_inventory SET quantity = quantity + ?, updated_by = ? WHERE id = ? AND farm_id = ?")
                   ->execute([$qty, $uid, $item_id, $fid]);
                if ($supplier !== '') {
                    $db->prepare("UPDATE feed_inventory SET supplier = ? WHERE id = ? AND farm_id = ?")->execute([$supplier, $item_id, $fid]);
                }
                $db->prepare("INSERT INTO inventory_transactions (farm_id,item_type,item_id,item_name,transaction_type,quantity,unit,unit_cost,total_value,notes,recorded_by) VALUES (?,?,?,?,?,?,?,?,?,?,?)")
                   ->execute([$fid, 'feed', $item_id, $item['item_name'], 'purchase', $qty, $item['unit'], $buy_price ?: null, $buy_price > 0 ? round($qty * $buy_price, 2) : null, $notes_txt ?: null, $uid]);
                if ($buy_price > 0) {
                    $db->prepare("INSERT INTO finance_transactions (farm_id,type,category,amount,related_module,reference_id,transaction_date,recorded_by,notes) VALUES (?,?,?,?,?,?,?,?,?)")
                       ->execute([$fid, 'expense', 'Feed Purchase', round($qty * $buy_price, 2), 'feed', $item_id, $pur_date, $uid, "Purchased {$qty} {$item['unit']} of {$item['item_name']}" . ($supplier ? " from {$supplier}" : '')]);
                }
                $db->commit();
                flashMessage('success', "Added {$qty} {$item['unit']} to {$item['item_name']} stock.");
                redirect('/modules/inventory/feed.php?tab=stock');
            } catch (Throwable $e) {
                $db->rollBack();
                error_log('[inv_feed_purchase] ' . $e->getMessage());
    ...
    if ($action === 'sell_stock') {
        ...
        if (empty($errors)) {
            ...
            $cost   = (float)($item['purchase_price'] ?? 0) > 0 ? round($qty * (float)$item['purchase_price'], 2) : 0.0;
            ...
            try {
                ...
                $db->prepare("INSERT INTO inventory_transactions (farm_id,item_type,item_id,item_name,transaction_type,quantity,unit,unit_cost,total_value,reference_type,reference_id,notes,recorded_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)")
                   ->execute([$fid, 'feed', $item_id, $item['item_name'], 'sale', $qty, $item['unit'], $ppu, $total, 'feed_sale', $sale_id, $buyer_name ? "Buyer: {$buyer_name}" : null, $uid]);
                $db->prepare("INSERT INTO finance_transactions (farm_id,type,category,amount,related_module,reference_id,transaction_date,recorded_by,notes) VALUES (?,?,?,?,?,?,?,?,?)")
                   ->execute([$fid, 'income', 'Feed Sales', $total, 'feed', $sale_id, $sale_date, $uid, "{$qty} {$item['unit']} of {$item['item_name']}" . ($buyer_name ? " to {$buyer_name}" : '')]);
                auditLog($uid, 'FEED_SALE', 'feed_sales', $sale_id, [], ['item' => $item['item_name'], 'qty' => $qty, 'total' => $total]);
                $db->commit();
                flashMessage('success', "Sold {$qty} {$item['unit']} of {$item['item_name']} for " . number_format($total, 2) . " BDT.");
                redirect('/modules/inventory/feed.php?tab=stock');
            } catch (Throwable $e) {
                $db->rollBack();
                error_log('[inv_feed_sell] ' . $e->getMessage());
                $errors[] = 'Database error. Please try again.';
            }
        }
        $tab = 'sell';
    ...
    if ($action === 'delete_item') {
        ...
        if ($row && (float)$row['quantity'] > 0) {
            flashMessage('error', 'Cannot delete item with stock remaining. Set quantity to 0 first.');
        } else {
            $db->prepare("DELETE FROM feed_inventory WHERE id = ? AND farm_id = ?")->execute([$item_id, $fid]);
...
// ── Fetch stock data ──────────────────────────────────────────────────────────
$stock_q = $db->prepare("SELECT * FROM feed_inventory WHERE farm_id = ? ORDER BY item_name");
$stock_q->execute([$fid]);
$all_stock = $stock_q->fetchAll();

$total_cost_val = 0; $total_sell_val = 0; $low_stock_count = 0; $expiring_count = 0;
foreach ($all_stock as $s) {
    $total_cost_val += (float)$s['quantity'] * (float)($s['purchase_price'] ?? 0);
    $total_sell_val += (float)$s['quantity'] * (float)($s['selling_price'] ?? $s['purchase_price'] ?? 0);
    if ((float)$s['reorder_threshold'] > 0 && (float)$s['quantity'] <= (float)$s['reorder_threshold']) $low_stock_count++;
    if (!empty($s['expiry_date']) && $s['expiry_date'] <= date('Y-m-d', strtotime('+30 days'))) $expiring_count++;
}

$history_q = $db->prepare("SELECT it.*, u.name AS recorder FROM inventory_transactions it LEFT JOIN users u ON u.id=it.recorded_by WHERE it.farm_id=? AND it.item_type='feed' ORDER BY it.created_at DESC LIMIT 100");
$history_q->execute([$fid]);
$history_rows = $history_q->fetchAll();

$sales_q = $db->prepare("SELECT fs.*, u.name AS recorder FROM feed_sales fs LEFT JOIN users u ON u.id=fs.recorded_by WHERE fs.farm_id=? ORDER BY fs.sale_date DESC, fs.id DESC LIMIT 50");
$sales_q->execute([$fid]);
$recent_sales = $sales_q->fetchAll();

$edit_item = null;
if ($tab === 'edit' && isset($_GET['item_id'])) {
    $ei = $db->prepare("SELECT * FROM feed_inventory WHERE id = ? AND farm_id = ?");
...
if (($tab === 'sell' || $tab === 'purchase') && isset($_GET['item_id'])) {
    $pi = $db->prepare("SELECT * FROM feed_inventory WHERE id = ? AND farm_id = ?");
    $pi->execute([(int)$_GET['item_id'], $fid]);
    $pre_item = $pi->fetch() ?: null;
}

$page_title = 'Feed Stock';
$active_nav = 'inv_feed';
...
<div class="page-header" style="flex-wrap:wrap;gap:.75rem">
    <div>
        <h1 class="page-title">Feed Stock</h1>
        <p class="page-subtitle">Manage feed inventory — track stock, purchase, and sell</p>
    </div>
...
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:1rem;margin-bottom:1.25rem">
    <div style="background:#fff;border:1px solid #e5e7eb;border-radius:10px;padding:1rem;text-align:center">
        ...
        <div style="font-size:.78rem;color:#6b7280">Feed Items</div>
    </div>
    <div style="background:#fff;border:1px solid #e5e7eb;border-radius:10px;padding:1rem;text-align:center">
        <div style="font-size:1.25rem;font-weight:700;color:#166534"><?= number_format($total_cost_val, 0) ?> ৳</div>
        <div style="font-size:.78rem;color:#6b7280">Stock Cost Value</div>
    </div>
    <div style="background:#fff;border:1px solid #e5e7eb;border-radius:10px;padding:1rem;text-align:center">
        <div style="font-size:1.25rem;font-weight:700;color:#0369a1"><?= number_format($total_sell_val, 0) ?> ৳</div>
    ...
    <div style="background:#fff7ed;border:1px solid #fed7aa;border-radius:10px;padding:1rem;text-align:center">
        <div style="font-size:1.5rem;font-weight:700;color:#c2410c"><?= $low_stock_count ?></div>
        <div style="font-size:.78rem;color:#c2410c">Low Stock Alerts</div>
    </div>
    <?php endif; ?>
    <?php if ($expiring_count > 0): ?>
    <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:10px;padding:1rem;text-align:center">
        <div style="font-size:1.5rem;font-weight:700;color:#dc2626"><?= $expiring_count ?></div>
        <div style="font-size:.78rem;color:#dc2626">Expiring &lt; 30 Days</div>
    </div>
    <?php endif; ?>
</div>
...
<div class="card">
    <div class="card-body" style="padding:0">
        ...
        <p style="text-align:center;padding:2.5rem;color:#9ca3af">No feed items yet. <a href="?tab=add">Add your first item.</a></p>
        <?php else: ?>
        ...
        <table style="width:100%;border-collapse:collapse;font-size:.85rem">
            <thead><tr style="background:#f9fafb">
                ...
                <th style="padding:.6rem .75rem;text-align:right;border-bottom:2px solid #e5e7eb">Stock Value</th>
            ...
            <?php foreach ($all_stock as $s):
                ...
                $is_expiring = !empty($s['expiry_date']) && $s['expiry_date'] <= date('Y-m-d', strtotime('+30 days'));
            ...
            <tr style="border-bottom:1px solid #f3f4f6;background:<?= $row_bg ?>">
                <td style="padding:.5rem .75rem;font-weight:600">
                    <?= e($s['item_name']) ?>
                    <?php if ($is_low): ?><span style="background:#fed7aa;color:#9a3412;font-size:.65rem;font-weight:700;padding:.1rem .35rem;border-radius:4px;margin-left:.3rem">LOW</span><?php endif; ?>
                </td>
                <td style="padding:.5rem .75rem;color:#6b7280"><?= e($s['category'] ?? '—') ?></td>
                ...
                <td style="padding:.5rem .75rem;text-align:right"><?= $s['purchase_price'] ? number_format((float)$s['purchase_price'], 2) : '—' ?></td>
                <td style="padding:.5rem .75rem;text-align:right"><?= $s['selling_price'] ? number_format((float)$s['selling_price'], 2) : '—' ?></td>
                <td style="padding:.5rem .75rem;text-align:right;font-weight:600;color:#166534"><?= $s['purchase_price'] ? number_format((float)$s['quantity'] * (float)$s['purchase_price'], 0) . ' ৳' : '—' ?></td>
                <td style="padding:.5rem .75rem">
                    <?php if ($s['expiry_date']): ?>
                    <span style="color:<?= $is_expired ? '#dc2626' : ($is_expiring ? '#d97706' : '#6b7280') ?>;font-size:.82rem"><?= e(formatDate($s['expiry_date'])) ?><?= $is_expired ? ' ⚠ Expired' : ($is_expiring ? ' ⚠' : '') ?></span>
                    <?php else: ?>—<?php endif; ?>
                </td>
                <td style="padding:.5rem .75rem">
...
<div class="card" style="max-width:680px">
    <div class="card-header"><span class="card-title"><?= $tab === 'edit' ? 'Edit Feed Item' : 'Add New Feed Item' ?></span></div>
    <div class="card-body">
        <form method="POST">
            <?= csrfField() ?>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem">
                ...
                <div class="form-group">
                    ...
                    <select name="category" class="form-control">
                        ...
                        <?php foreach (['Hay', 'Silage', 'Grain', 'Concentrate', 'Mineral Supplement', 'Protein Supplement', 'Roughage', 'Other'] as $cat):
                            $sel = ($edit_item['category'] ?? $_POST['category'] ?? '') === $cat ? 'selected' : ''; ?>
                        <option value="<?= $cat ?>" <?= $sel ?>><?= $cat ?></option>
                        <?php endforeach; ?>
                        <?php $cv = $edit_item['category'] ?? ''; if ($cv && !in_array($cv, ['Hay','Silage','Grain','Concentrate','Mineral Supplement','Protein Supplement','Roughage','Other'])): ?>
                        <option value="<?= e($cv) ?>" selected><?= e($cv) ?></option>
                        <?php endif; ?>
                    </select>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <select name="unit" class="form-control">
                        <?php foreach (['kg', 'g', 'ton', 'L', 'bags', 'bales', 'bundles', 'pieces'] as $u):
                            $sel = ($edit_item['unit'] ?? $_POST['unit'] ?? 'kg') === $u ? 'selected' : ''; ?>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="purchase_price" class="form-control" step="0.01" min="0" value="<?= e($edit_item['purchase_price'] ?? $_POST['purchase_price'] ?? '') ?>">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <label class="form-label">Purchase Date</label>
                    <input type="date" name="purchase_date" class="form-control" value="<?= e($edit_item['purchase_date'] ?? '') ?>">
                </div>
                <?php endif; ?>
                <div class="form-group">
        ...
        <form method="POST" id="purchForm">
            ...
            <div class="form-group">
                ...
                <select name="item_id" id="pur_item" class="form-control" required>
                    ...
                    <?php foreach ($all_stock as $s): ?>
                    <option value="<?= $s['id'] ?>" <?= ($pre_item && $pre_item['id'] == $s['id']) ? 'selected' : '' ?> data-unit="<?= e($s['unit']) ?>" data-stock="<?= $s['quantity'] ?>">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem">
                ...
                <div class="form-group">
                    <label class="form-label">Unit</label>
                    <input type="text" name="unit_display" id="pur_unit_d" class="form-control" readonly style="background:#f9fafb" value="<?= e($pre_item['unit'] ?? '') ?>">
                </div>
            </div>
            <div class="form-group">
                <label class="form-label">Cost Price / Unit (৳)</label>
                <input type="number" name="purchase_price" id="pur_price" class="form-control" step="0.01" min="0" value="<?= e($_POST['purchase_price'] ?? '') ?>" placeholder="Optional">
            </div>
            <div id="pur_total_box" style="display:none;background:#f0fdf4;border:1px solid #86efac;border-radius:8px;padding:.6rem .75rem;font-weight:600;color:#166534;margin-bottom:.75rem">Total Cost: —</div>
            <div class="form-group">
                <label class="form-label">Supplier</label>
...
<div class="card" style="max-width:640px">
    <div class="card-header"><span class="card-title">Sell Feed Stock</span></div>
    <div class="card-body">
            ...
            <div class="form-group">
                ...
                <select name="item_id" id="sell_item" class="form-control" required>
                    ...
                    <option value="<?= $s['id'] ?>"
                        ...
                        data-cost="<?= (float)($s['purchase_price'] ?? 0) ?>"
                        data-sell="<?= (float)($s['selling_price'] ?? $s['purchase_price'] ?? 0) ?>">
                        <?= e($s['item_name']) ?> (<?= number_format((float)$s['quantity'], 1) ?> <?= e($s['unit']) ?> available)
                    </option>
                    <?php endforeach; ?>
                </select>
    ...
    <div class="card-body" style="padding:0">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:.85rem">
            <thead><tr style="background:#f9fafb">
                ...
                <th style="padding:.6rem .75rem;text-align:left;border-bottom:2px solid #e5e7eb">Item</th>
                <th style="padding:.6rem .75rem;text-align:left;border-bottom:2px solid #e5e7eb">Type</th>
                ...
                <th style="padding:.6rem .75rem;border-bottom:2px solid #e5e7eb">Recorded By</th>
            </tr></thead>
            <tbody>
            ...
            <tr style="border-bottom:1px solid #f3f4f6">
                <td style="padding:.5rem .75rem;white-space:nowrap;font-size:.8rem"><?= e(date('d M Y H:i', strtotime($h['created_at']))) ?></td>
                ...
                <td style="padding:.5rem .75rem;text-align:right"><?= $h['unit_cost'] ? number_format((float)$h['unit_cost'], 2) : '—' ?></td>
...
<div class="card">
    ...
    <div class="card-body" style="padding:0">
        ...
        <p style="text-align:center;padding:2rem;color:#9ca3af">No feed sales yet.</p>
        <?php else: ?>
        <div style="overflow-x:auto">
        <table style="width:100%;border-collapse:collapse;font-size:.85rem">
            <thead><tr style="background:#f9fafb">
            ...
            <tr style="border-bottom:1px solid #f3f4f6">
                ...
                <td style="padding:.5rem .75rem;text-align:right;font-weight:600;color:<?= (float)($s['profit'] ?? 0) >= 0 ? '#166534' : '#dc2626' ?>">
...
(function(){
    ...
    var pur_unit_d = document.getElementById('pur_unit_d');
    var pur_qty = document.getElementById('pur_qty');
    var pur_price = document.getElementById('pur_price');
    var pur_total = document.getElementById('pur_total_box');
    if (pur_item) {
        pur_item.addEventListener('change', function(){
            var o = this.options[this.selectedIndex];
            if (pur_unit_d) pur_unit_d.value = o.dataset.unit || '';
            calcPur();
        });
        if (pur_qty) pur_qty.addEventListener('input', calcPur);
        if (pur_price) pur_price.addEventListener('input', calcPur);
        function calcPur(){
            var q = parseFloat(pur_qty ? pur_qty.value : 0)||0;
            var p = parseFloat(pur_price ? pur_price.value : 0)||0;
            if (pur_total) { if(q&&p){pur_total.textContent='Total Cost: ৳'+(q*p).toFixed(2);pur_total.style.display='block';}else{pur_total.style.display='none';}}
        }
    }
    ...
    if (sell_item) {
        sell_item.addEventListener('change', function(){
            var o = this.options[this.selectedIndex];
            if (sell_info && o.value) { sell_info.textContent = 'Available: '+o.dataset.stock+' '+o.dataset.unit; }
            if (sell_ppu && o.dataset.sell) { sell_ppu.value = o.dataset.sell; }
            calcSell();
        });
        if (sell_qty) sell_qty.addEventListener('input', calcSell);
        if (sell_ppu) sell_ppu.addEventListener('input', calcSell);
        function calcSell(){
            var o = sell_item.options[sell_item.selectedIndex];
            var q = parseFloat(sell_qty ? sell_qty.value : 0)||0;
            var p = parseFloat(sell_ppu ? sell_ppu.value : 0)||0;
            var c = parseFloat(o.dataset.cost)||0;
            if (sell_total) sell_total.textContent = 'Total: ' + (q&&p ? '৳'+(q*p).toFixed(2) : '—');
            if (sell_profit && q && p && c) {
                var profit = (p-c)*q;
                sell_profit.textContent = 'Est. Profit: '+(profit>=0?'+':'')+'৳'+profit.toFixed(2);
                sell_profit.style.color = profit >= 0 ? '#166534' : '#dc2626';
            } else if (sell_profit) { sell_profit.textContent = ''; }
        }
    }
})();

function toggleAmtPaid(){
    var s = document.getElementById('sell_pay_status');
    var b = document.getElementById('amt_paid_box');
    if (b) b.style.display = (s && s.value === 'partial') ? 'block' : 'none';
}
toggleAmtPaid();
</script>
